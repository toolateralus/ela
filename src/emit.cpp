...
#warning "Re enable me";
      ...
      code << "int main(int argc, char** argv) { __ela_main_(); }";
...
void Emitter::visit_parameters(const std::vector<AST_Parameter_Declaration> &parameters, bool is_varargs) {
  (*ss) << "(";
  size_t i = 0;
...
void Emitter ::visit_function_declaration(AST *node) {
  auto emit_function_signature_and_body = [&](const std::string &name) {
    ...
    visit_parameters(node->function.parameters, node->function.is_varargs);
...
void Emitter ::emit_lambda(AST *node) {
  ...
  visit_parameters(node->lambda.parameters, false);
