...
void fetch_compiler_features_for_struct_declaration_concrete(ASTStructDeclaration *node, Type *attribute_feature_type) {
  for (const auto &attribute : node->attributes) {
    if (attribute.tag == ATTRIBUTE_COMPILER_FEATURE) {
      const InternedString feature = get_identifier_from_attribute_arguments(node->source_range, attribute);
...
void Typer::visit_struct_declaration(ASTStructDeclaration *node, bool generic_instantiation,
  ...
  if (generic_instantiation) {
    ...
    auto symbol = ctx.scope->local_find(node->name);
    if (symbol && type_is_valid(symbol->type)) {
      type = symbol->type;
    }
  ...
  for (auto subunion : node->subtypes) {
    ...
    for (const auto &field : subunion->members) {
      ...
      info->scope->insert(field.name, field.type->resolved_type, nullptr, MUT);
    }
  ...
  for (const auto variant : variants) {
    switch (variant.index()) {
      case ALIAS_VARIANT_INDEX: {
        ...
        info->scope->local_find(name)->type.choice = node;
      } break;
...
void Typer::visit_impl_declaration(ASTImpl *node, bool generic_instantiation, std::vector<Type *> generic_args) {
  ...
  for (const auto &method : node->methods) {
    ...
    if (auto symbol = type_scope->local_find(method->name)) {
  ...
  if (trait_ty) {
    ...
    for (auto trait_method : trait->methods) {
      ...
      if (auto impl_symbol = impl_scope.local_lookup(method->name)) {
        method->accept(this);
        if (!impl_method_matches_trait(method->type, impl_symbol->resolved_type)) {
          if (method->type != Type::INVALID_TYPE && impl_symbol->resolved_type != Type::INVALID_TYPE) {
            throw_error(std::format("method \"{}\" doesn't match trait.\nexpected {},\ngot {}", method->name,
                                    method->type->to_string(), impl_symbol->resolved_type->to_string()),
                        node->source_range);
          } else {
...
void Typer::visit_trait_declaration(ASTTraitDeclaration *node, bool generic_instantiation,
  ...
  auto id = ctx.scope->find_type(node->name, {});
  ...
  if (was_forward_declared) {
    type = ctx.scope->find_type(node->name, {});
  } else {
  ...
  if (auto symbol = ctx.scope->find(node->name)) {
    type->generic_base_type = symbol->type;
  }
...
void Typer::visit(ASTLambda *node) {
  ...
  ctx.scope->insert(node->unique_identifier, type, node, MUT);
}
...
void Typer::visit(ASTVariable *node) {
  ...
  if (node->type->type->is_kind(TYPE_TRAIT)) {
    throw_error(
  ...
  if (node->is_local) {
    ...
    ctx.scope->insert(node->name, variable_type, node->value.get(), node->mutability, node);
  }
...
void Typer::visit(ASTFor *node) {
  ...
  if (iterable_type->implements(g_Iterable_trait_type)) {  // can return an iterator.
    ...
    auto symbol = scope->local_find("iter");
    auto symbol_ty = symbol->type;
    ...
    iter_ty = symbol->type->info->as<FunctionTypeInfo>()->return_type;
    auto option = iter_ty;
    ...
    auto symbol = iterable_type->info->scope->local_find("next");
    iter_ty = symbol->type->info->as<FunctionTypeInfo>()->return_type;
    auto option = iter_ty;
    iter_ty = option->generic_args[0];
  } else {
...
void Typer::visit(ASTCall *node) {
  ...
  if (symbol && symbol->is_function) {
    ...
    ENTER_EXPECTED_TYPE(symbol->type);
    tuple.accept(this);
...
void Typer::visit(ASTType_Of *node) {
  static auto type_ptr = ctx.scope->find_type("Type", {{TYPE_EXT_POINTER_CONST}});
  node->target->accept(this);
  ...
  switch (node->tag) {
    ...
    case ASTLiteral::String: {
      ...
      if (node->is_c_string) {
        ...
        static Type *type = ctx.scope->find_type("str", {});
        node->resolved_type = type;
      }
      return;
    }
    ...
    case ASTLiteral::Null:
      ...
      node->resolved_type = ctx.scope->find_type("void", {{TYPE_EXT_POINTER_CONST}});
      return;
...
void Typer::visit(ASTDotExpr *node) {
  ...
  if (auto member = base_scope->local_find(identifier)) {
    /*
      TODO Resolve generic arguments for dot expression?
    */
    node->resolved_type = member->type;
  } else {
...
// For now, redefinitions from module "appends" are untracked.
  ...
  if (Symbol *mod = ctx.scope->find(node->module_name)) {
    if (!mod->is_module) {
      throw_error(
...
*/
Type *Scope::find_or_instantiate_dyn_type(Type *trait_type, SourceRange range, Typer *typer) {
...
Nullable<Symbol> Context::get_symbol(ASTNode *node) {
  switch (node->get_node_type()) {
    ...
    case AST_NODE_DOT_EXPR: {
      ...
      auto symbol = type->info->scope->local_find(identifier);

      // Implicit dereference, we look at the base scope.
      if (!symbol && type->is_pointer()) {
        type = type->get_element_type();
        symbol = type->info->scope->local_find(identifier);
      }
      return symbol;
    }
    default:
