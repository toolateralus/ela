...
Value evaluate_constexpr(ASTExpr *node, Context &ctx) {
  ...
  case AST_NODE_IDENTIFIER: {
    throw_error(
        "Not yet implemented: constexprs will soon have identifier access.",
        node->source_range);
  }
  case AST_NODE_BIN_EXPR: {
    ...
    case TType::Div:
      if ((right == Value::Int(0)).is_truthy()) {
        throw_error("Division by zero in constant expression",
                    node->source_range);
      }
      return left / right;
    case TType::Modulo:
      if ((right == Value::Int(0)).is_truthy()) {
        throw_error("Modulo by zero in constant expression",
                    node->source_range);
      }
      return left % right;
    case TType::Not:
    ...
    case TType::GE:
      return left >= right;
    default:
      throw_error("Invalid binary operator in constant expression",
                  node->source_range);
    }
  ...
  case AST_NODE_UNARY_EXPR: {
    ...
    case TType::BitwiseNot:
      return ~operand;
    default:
      throw_error("Invalid unary operator in constant expression",
                  node->source_range);
    }
  } break;
  case AST_NODE_LITERAL: {
    ...
    case ASTLiteral::Bool:
      ...
      break;
    default:
      throw_error("Invalid literal type in constant expression",
                  node->source_range);
    }
  } break;
  default:
