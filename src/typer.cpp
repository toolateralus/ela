...
int assert_type_can_be_assigned_from_init_list(ASTInitializerList *node, int declaring_type) {
  ...
  if (type->is_kind(TYPE_SCALAR)) {
    ...
    if (node->types.size() > 1) {
      throw_error(
          "You can only initialize one field of a union with an "
          "initializer list",
          node->source_range);
    }
...
Nullable<Symbol> Typer::get_symbol(ASTNode *node) {
  switch (node->get_node_type()) {
    ...
    case AST_NODE_DOT_EXPR: {
      ...
      if (type->get_ext().is_array()) return nullptr;

      if (type->get_ext().is_map()) return nullptr;
...
std::any Typer::visit_function_declaration(ASTFunctionDeclaration *node, bool generic_instantation,
  ...
  if (generic_instantation) {
    node->generic_instantiations.push_back({generic_args, node, type_id});
...
std::any Typer::visit(ASTFor *node) {
  ...
  if (range_type_id == string_type()) {
    ...
    throw_error(
        "Cannot iterate with a range-based for loop over a "
        "non-collection type.",
        node->source_range);
  }
    ...
    if (auto declaring_node = dynamic_cast<ASTFunctionDeclaration *>(symbol->declaring_node.get())) {
      if (!declaring_node->generic_parameters.empty()) {
        ...
        if (type == nullptr) {
          ...
          visit_function_declaration(declaring_node, true, gen_args);
          type = find_generic_function(declaring_node, gen_args);
          if (type == nullptr) {
            throw_error("Internal Compiler Error: could not find type of generic instantiation after instatiating it",
                        node->source_range);
          }
        }
...
std::any Typer::visit(ASTType *node) {
  ...
  if (!node->tuple_types.empty()) {
    ...
    for (const auto &t : node->tuple_types) types.push_back(int_from_any(t->accept(this)));
    ...
    case ASTLiteral::Integer: {
      ...
      if (declaring_or_assigning_type != -1) {
        ...
        if (type->is_kind(TYPE_SCALAR) && type_is_numerical(type)) {
          auto info = (type->get_info()->as<ScalarTypeInfo>());
          if (info->is_integral) return type->id;
        }
      }
      return s32_type();
    }
...
std::any Typer::visit(ASTScopeResolution *node) {
  ...
  if (node->base == nullptr) {
    ...
    if (!found) throw_error(std::format("Unable to find enum variant {}", node->member_name), node->source_range);
  }
 ...
 */
  ...
  if (ext.is_map()) {
    assert_types_can_cast_or_equal(subscript, ext.key_type, node->source_range, "expected : {}, got {}",
...
std::any Typer::visit(ASTAllocate *node) {
  ...
  if (node->arguments) node->arguments.get()->accept(this);
