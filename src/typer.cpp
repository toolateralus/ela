...
void handle_generic_error_if_failed(GenericInstantiationErrorUserData *data, const SourceRange &range) {
  reset_panic_handler();
  if (data->failed) {
...
int Typer::find_generic_type_of(const InternedString &base, const std::vector<int> &generic_args,
  ...
  if (!symbol || !symbol->declaring_node) {
  ...
  handle_generic_error_if_failed(&data, source_range);
