...
int global_find_type_id(std::vector<int> &tuple_types, const TypeExtensions &type_extensions) {
  for (int i = 0; i < type_table.size(); ++i) {
    ...
    if (info->types.size() != tuple_types.size()) continue;

    for (int i = 0; i < info->types.size(); ++i)
      if (info->types[i] != tuple_types[i]) goto end_of_loop;
...
ConversionRule type_conversion_rule(const Type *from, const Type *to, const SourceRange &source_range) {
  if (!from || !to) {
    throw_error(
        "Internal Compiler Error: type was null when checking type "
        "conversion rules",
        source_range);
  }
  ...
  if (from->id == to->id) return CONVERT_NONE_NEEDED;
  ...
  if (from->is_kind(TYPE_SCALAR) && from->get_ext().has_no_extensions() && to->is_kind(TYPE_SCALAR) &&
    ...
    if (type_is_numerical(from) && type_is_numerical(to)) {
      if (numerical_type_safe_to_upcast(from, to)) return CONVERT_IMPLICIT;
  ...
  const auto implicit_fixed_array_to_ptr_cast = [&]() {
    // not array, return.
    if (!from->get_ext().is_fixed_sized_array()) return false;

    if (!to->get_ext().is_pointer()) return false;
