...
;
;
float randf() { return (float(rand()) / float(Random_Max)); };
...
union Vec4 {
  ...
  float sqr_mag() { return ((((x * x) + (y * y)) + (z * z)) + (w * w)); };
  struct {
    float x;
    float y;
    float z;
...
;
Vec4 Vec4_one() { return Vec4(1.0f); };
Vec4 Vec4_zero() { return Vec4(0.0f); };
union Vec3 {
  ...
  float sqr_mag() { return (((x * x) + (y * y)) + (z * z)); };
  struct {
    float x;
    float y;
    float z;
...
;
Vec3 Vec3_one() { return Vec3(1.0f); };
Vec3 Vec3_zero() { return Vec3(0.0f); };
union Vec2 {
...
;
Vec2 Vec2_one() { return Vec2(1.0f); };
Vec2 Vec2_zero() { return Vec2(0.0f); };
union Mat4 {
  float values[16]{float(), float(), float(), float(), float(), float(),
...
struct Window {
  ...
  Window(int width, int height, char *title) {
    ...
    (handle = (GLFWwindow *)glfwCreateWindow(width, height, title, nullptr,
                                             nullptr));
    if ((handle == nullptr)) {
...
_array<Vec2> positions = {Vec2((-1.0f), (-1.0f)), Vec2(1.0f, (-1.0f)),
                          Vec2(1.0f, 1.0f), Vec2((-1.0f), 1.0f)};
Vec2 normal = Vec2(0.0f, 0.0f);
_array<Vec2> uvs = {Vec2(0.0f, 0.0f), Vec2(1.0f, 0.0f), Vec2(1.0f, 1.0f),
                    Vec2(0.0f, 1.0f)};
...
s8 run_game() {
  ...
  glfwSwapInterval(1);
  {
    ...
    glBufferData(GL_ARRAY_BUFFER, (sizeof(Vertex) * vertexCount),
